...
namespace cnxx {
    namespace {
        ...
        CDVector forceVariant1Part(const HubbardFermiMatrix &hfm, const CDVector &phi,
                                   const CDSparseMatrix &k, const bool hole) {
            ...
            CDSparseMatrix f = hfm.F(nt-1, phi, hole, true);
            lefts.emplace_back(f*k);
            // other terms
            for (std::size_t t = nt-2; t != 0; --t) {
                hfm.F(f, t, phi, hole, true);
                lefts.emplace_back(f*k*lefts.back());
            }
            // full A^-1
            hfm.F(f, 0, phi, hole, true);
    ...
    CDVector HFA::force(const CDVector &phi) {
        if (_variant2)
            ...
            return -1.i*(forceVariant1Part(_hfm, phi, _kp, false)
                         - forceVariant1Part(_hfm, phi, _kh, true));
    }
