...
namespace isle{
  ...
  CDMatrix mult_CDMatrix_wrapper(const CDMatrix &a, const CDMatrix &b, const std::size_t N) {
    ...
    CHECK_CU_ERR(cudaMalloc(&A, N*N*sizeof(cuDoubleComplex)));
    CHECK_CU_ERR(cudaMalloc(&B, N*N*sizeof(cuDoubleComplex)));
    CHECK_CU_ERR(cudaMalloc(&C, N*N*sizeof(cuDoubleComplex)));

    CHECK_CU_ERR(cudaMemcpy(A, cast_cmpl(a.data()), N*N*sizeof(cuDoubleComplex), cudaMemcpyHostToDevice));
    CHECK_CU_ERR(cudaMemcpy(B, cast_cmpl(b.data()), N*N*sizeof(cuDoubleComplex), cudaMemcpyHostToDevice));
    ...
    CHECK_CU_ERR(cudaMemcpy(res.data(), cast_cmpl(C), N*N*sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost));
