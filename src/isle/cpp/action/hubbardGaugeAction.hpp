...
namespace isle {
    namespace action {
        ...
        */
        ...
        class HubbardGaugeActionML : public Action {
        public:
            double utilde;  ///< Parameter \f$\tilde{U}\f$.

            /// Set \f$\tilde{U}\f$.
            explicit HubbardGaugeActionML(const double utilde_) : utilde{utilde_} { }

            HubbardGaugeActionML(const HubbardGaugeActionML &other) = default;
            HubbardGaugeActionML &operator=(const HubbardGaugeActionML &other) = default;
            HubbardGaugeActionML(HubbardGaugeActionML &&other) = default;
            HubbardGaugeActionML &operator=(HubbardGaugeActionML &&other) = default;
            ~HubbardGaugeActionML() override = default;

            /// Evaluate the %Action for given auxilliary field phi.
            std::complex<double> eval(const Vector<std::complex<double>> &phi) const override;

            /// Calculate force for given auxilliary field phi.
            // Dummy construct  to suit isle format here since the NN predicts Gauge and Fermi forces 
            Vector<std::complex<double>> force(const Vector<std::complex<double>> &phi) const override;
        };
    }  // namespace action
}  // namespace isle
