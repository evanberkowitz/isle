...
#
# PYBIND11_FOUND         - True if Pybind11 was found PYBIND11_CXX_FLAGS     -
# Pybind11 compiler flags PYBIND11_INCLUDE_DIRS  - Pybind11 include directories
# PYBIND11_LINKER_FLAGS  - Pybind11 linker flags PYBIND11_LIBRARIES     -
# Pybind11 libraries PYBIND11_LIB_SUFFIX    - Library extension suffix for this
# version of python
#
...
function(python_config args output)
  ...
  if(NOT "${rc}" STREQUAL "0")
    message(
      FATAL_ERROR
        "Command '${PYTHON_CONFIG} ${args}' could not be executed. Return code: ${rc}"
    )
...
# Split input into lists of flags and paths where all paths are have a certain
# prefix in the input (stripped from output).
...
if(NOT "${RC}" STREQUAL "0")
  message(
    FATAL_ERROR
      "Could not execute python to get pybind11 modules. Return code: ${RC}")
endif()
# turn it into a list
...
foreach(lib IN LISTS AUX_LIBS)
  ...
  find_library(
    aux_path ${lib}
    PATHS ${PYBIND11_LIB_PATHS}
    NO_DEFAULT_PATH)
  find_library(aux_path ${lib} PATHS ${PYBIND11_LIB_PATHS};${LD_LIBRARY_PATH})
  if(NOT aux_path)
    message(
      FATAL_ERROR
        "Did not find library ${lib}. Try to adjust PYBIND11_LIB_PATHS")
  endif()
